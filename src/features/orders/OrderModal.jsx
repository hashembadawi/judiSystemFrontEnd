    ...
    <div className="modal-backdrop" role="dialog" aria-modal="true">
      <section className="modal-card order-modal-card">
        <header className="order-modal-header">
          <div>
            <p className="order-modal-tag">طلبية جديدة</p>
            <h4>{form.id ? 'تعديل الطلبية' : 'اضافة طلبية جديدة'}</h4>
          </div>
          <button
            type="button"
            className="ghost-btn order-modal-close-btn"
            onClick={onClose}
            aria-label="اغلاق"
          >
            ×
                ...
                {form.details.map((detail, detailIndex) => (
                  <div className="order-detail-card" key={`${detail.id || detailIndex}-detail`}>
                    <div className="order-detail-grid">
                      <div className="field-group">
                        <label htmlFor={`fabricGender-${detailIndex}`}>جنس القماش</label>
                        <select
                      ...
                      <div className="field-group">
                        <label htmlFor={`fabricGSM-${detailIndex}`}>GR</label>
                        <input
